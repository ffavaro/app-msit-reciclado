...
export async function GET(request: Request) {
  try {
    ...
    get_exp_query = 'SELECT * FROM zonas'
...
export async function POST(request: Request) {
  try {
    ...
    insert_query = 'INSERT INTO zonas (nombre, descripcion) VALUES (?, ?)'

    // we can use this array to pass parameters to the SQL query
    let values: any[] = [reqBody.nombre, reqBody.descripcion]
    // 4. exec the query and retrieve the results
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const connection = await mysql.createConnection(connectionParams)
    let update_query = 'UPDATE zonas SET nombre = ?, descripcion = ? WHERE id = ?'
    let values: any[] = [reqBody.nombre, reqBody.descripcion, id]
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const connection = await mysql.createConnection(connectionParams)
    let delete_query = 'DELETE FROM zonas WHERE id = ?'
    let values: any[] = [id]
